...
static void PerformMD4(const unsigned char *buf, const int length, unsigned char *digest)
{
	const int len = length / 64; // Number of full blocks.
	const int rem = length % 64; // Number of left over bytes.
...
unsigned Com_BlockChecksum(void *buffer, const int length)
{
	uint32_t digest[4];
