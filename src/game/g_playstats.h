...
#define GRAV_MOV				1.0		// Time to move from min to max grav values - server side.
#define MAX_GRAV				1.2		// Gravity flamethrower starts with - server side.
#define MIN_GRAV				0.12	// Gravity flamethrower maxes out at - server side.
#define ZEN_TIME				3.0		// Time for flamethrower to be at its max - server side - seconds.
#define FLAME_THROW_DAMAGE		10.0	// Damage flamethrower does at one hit.
#define FLAME_UPDATE_PER_SEC	0.2		// Frame update speed - server side.
...
#define FLAME_GRAV		(-675.0)				// Initial gravity value - client effects side. Should match cl_gravity->value.
...
#define ZEN_TIME_FX		(ZEN_TIME * 1000.0)		// Time for flamethrower to be at its max - client effects side - milliseconds.
#define GRAV_MOV_FX		(GRAV_MOV * 1000.0)		// Time to move from min to max grav values - client effects side - milliseconds.
