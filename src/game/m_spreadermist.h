...
extern edict_t* RadiusDamageEnt(edict_t* position_owner, edict_t* damage_owner, int damage, int delta_damage, float radius, float delta_radius, int dflags, float lifetime, float think_increment, const vec3_t origin, const vec3_t offset, qboolean attach);
