...
static void CreateLightning(struct client_entity_s* self, const centity_t* owner)
{
	...
	if (owner->current.number == fxi.cl->playernum + 1)
	{
		...
		fxi.Activate_Screen_Shake(4.0f, 800.0f, (float)fxi.cl->time, SHAKE_ALL_DIR); // 'current_time' MUST be cl.time, because that's what used by Perform_Screen_Shake() to calculate effect intensity/timing... --mxd.
	}
}
