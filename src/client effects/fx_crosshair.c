...
static qboolean CrosshairUpdate(client_entity_t* self, centity_t* owner) //mxd. Named 'FXDrawCrosshair' in original logic.
{
	self->flags |= (CEF_CULLED | CEF_DISAPPEARED);

	// Get new destination.
	byte type;
	if (fxi.Get_Crosshair(self->r.origin, &type))
	{
		self->r.frame = (type > 2 ? 0 : type);
		self->r.flags = (RF_TRANSLUCENT | RF_TRANS_ADD | RF_TRANS_ADD_ALPHA | RF_NODEPTHTEST);

		float alpha = 0.5f + fabsf(sinf((float)fx_time / 800.0f)) * 0.5f;
		alpha = Clamp(alpha, 0.0f, 1.0f) * 0.5f;

		self->alpha = alpha + 0.25f;
		self->r.scale = alpha;

		self->flags &= ~(CEF_CULLED | CEF_DISAPPEARED | CEF_NO_DRAW);
	}
