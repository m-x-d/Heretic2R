//
// ce_Main.c -- Named 'Main.c' in original logic --mxd.
//
...
#include "ce_DLight.h"
#include "ce_LightStyles.h" //mxd
#include "ce_Particles.h"
#include "ce_Utilities.h" //mxd
...
static void AddServerEntities(const frame_t* frame)
{
	...
	for (int pnum = 0; pnum < num_ents_to_add; pnum++)
	{
		...
		else
		{
			...
			//mxd. Special handling for monster_harpy. Can exceed cl_lerpdist2 during harpy_dive_end_move(), so allow it to move 200 units per frame instead of default 100.
			const float lerpdist_scaler = (effects & EF_FAST_MOVER ? 4.0f : 1.0f); 
