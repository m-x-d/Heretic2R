//
// fx_FireWall.c -- Named 'fx_Wall.c' in original logic --mxd.
//
...
static qboolean FireWormUpdate(client_entity_t* self, centity_t* owner) //mxd. Named 'FXFireWormThink' in original logic.
{
	...
	trace_t trace;
	fxi.Trace(self->r.origin, vec3_origin, vec3_origin, end_pos, CONTENTS_SOLID, CTF_CLIP_TO_WORLD | CTF_CLIP_TO_BMODELS, &trace); //mxd. +CTF_CLIP_TO_BMODELS flag.
