...
void RotateModelSegments(ModelSkeleton_t* skel, const int jointIndex, const int modelClusterIndex, const int anglesIndex, vec3_t* modelVerticies)
{
...
void LinearllyInterpolateJoints(ModelSkeleton_t* newSkel, const int newIndex, ModelSkeleton_t* oldSkel, const int oldIndex, ModelSkeleton_t* liSkel, const int liIndex, float move[3], float frontv[3], float backv[3])
{
	...
	R_LerpVert(newJoint->model.origin, oldJoint->model.origin, liJoint->model.origin, move, frontv, backv);

	// Linearly interpolate direction and up vectors, which will un-normalize them relative to their local origin.
	R_LerpVert(newJoint->model.direction, oldJoint->model.direction, liJoint->model.direction, move, frontv, backv);
	R_LerpVert(newJoint->model.up, oldJoint->model.up, liJoint->model.up, move, frontv, backv);
