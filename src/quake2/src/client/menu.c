...
void M_Init(void)
{
	...
	m_music_track = Cvar_Get("menu_music_track", va("%i", CDTRACK_MENU_MAIN), CVAR_ARCHIVE); //mxd
	...
	m_item_nightmare = Cvar_Get("m_item_nightmare", "Annihilation (Too hard)", 0); //H2: "Insane".
	...
	m_item_camera_rotation_lerp = Cvar_Get("m_item_camera_rotation_lerp", "Rotation Smoothing", 0); //mxd. "m_item_cameradamp" / "Camera Stiffness" in original logic.
