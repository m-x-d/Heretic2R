...
void SV_RemoveEffects(entity_state_t* ent, int fx_type);
void SV_CreateEffectEvent(byte event_id, entity_state_t* ent, int fx_type, int flags, const vec3_t origin, const char* format, ...);
void SV_RemoveEffectsEvent(byte event_id, entity_state_t* ent, int fx_type);
int SV_CreatePersistantEffect(const entity_state_t* ent, int fx_type, int flags, const vec3_t origin, const char* format, ...);
qboolean SV_RemovePersistantEffect(int fx_type, int call_from);
