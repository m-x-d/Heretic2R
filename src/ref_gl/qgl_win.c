...
#pragma region ========================== GLW FUNCTION POINTERS ==========================
...
#pragma region ========================== GL FUNCTION POINTERS ==========================
...
#pragma region ========================== GL FUNCTION CALLS LOGGING ==========================
...
{
	// MISSING: "update 3Dfx gamma irrespective of underlying DLL" block
...
void GLimp_EnableLogging(qboolean enable)
{
	if (enable)
	{
		if (!glw_state.log_fp)
		{
			...
			char time_str[26];
			asctime_s(time_str, sizeof(time_str), &newtime); //mxd. asctime -> asctime_s

			Com_sprintf(buffer, sizeof(buffer), "%s/gl.log", ri.FS_Gamedir());
			fopen_s(&glw_state.log_fp, buffer, "wt"); //mxd. fopen -> fopen_s
